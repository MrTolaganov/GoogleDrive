...
export default function SettingsPage() {
  ...
  const onSubmit = () => {
    const priceId = "price_1PLMeWJ1xkLdVRx2SQoMtuAr";
    const promise = axios
    ...
    <>
      ...
      <Tabs defaultValue="account" className=" mt-4">
        ...
        <TabsContent value="account">
          <UserProfile
            appearance={{ baseTheme: resolvedTheme === "dark" ? dark : shadesOfPurple }}
          />
        </TabsContent>
          ...
          <div className="flex space-x-4">
            ...
            <div className="border rounded-xl p-4 flex-1 px-12">
              <p className="text-lg font-bold">
                {subscription === "Basic" ? "Upgrade for benefits" : "Your plan benefits"}
              </p>
              <div className="grid grid-cols-2 mt-4">
                {planArray[1].options.split(", ").map(option => (
                  <div className="flex items-center pb-2 space-x-2" key={option}>
                    <BadgeCheck className="w-5 h-5 text-blue-500" />
                    <p className="opacity-75">{option}</p>
                  </div>
                ))}
              </div>
