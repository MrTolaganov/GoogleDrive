...
export default function ListItem({ item }: ListItemProps) {
  ...
  const onStartEditing = (e: React.MouseEvent<HTMLInputElement, MouseEvent>) => {
    e.stopPropagation();
    setIsEditing(true);
    setTimeout(() => {
  ...
  const onSave = () => {
    ...
    const promise = setDoc(ref, { ...item, name: value.length ? value : "Untitled" }).then(() => {
      setIsEditing(false);
      refresh();
    });
  ...
  return (
    ...
    >
        ...
        {isEditing ? (
          <div className="relative">
            <Input
              ...
              onKeyDown={onKeyDown}
            />
            <div className="absolute top-0 right-0 h-full flex items-center space-x-1">
              <Button size={"sm"} variant={"outline"} className="h-full" onClick={onSave}>
          ...
          <div className="flex items-center space-x-1" role="button" onDoubleClick={onStartEditing}>
            {item.size ? (
      ...
      <TableCell>{format(new Date(item.timestamp.seconds * 1000), "MMM dd, yyy")}</TableCell>
